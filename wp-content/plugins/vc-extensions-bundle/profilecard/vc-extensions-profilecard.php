...
if (!class_exists('VC_Extensions_ProfileCard')) {
    ...
    class VC_Extensions_ProfileCard {
        function VC_Extensions_ProfileCard() {
          wpb_map( array(
        ...
        function cq_vc_profilecard_func($atts, $content=null) {
          ...
          $output = '';
          $output .= '<div class = "profilecard-container '.$thumb_class.'" style="width:'.$width.'px;height:'.$height.'px;" data-image="'.aq_resize($image[0], $retina=="on"?$width*2:$width, $retina=="on"?$height*2:$height, true, true, true).'" data-width="'.$width.'" data-height="'.$height.'" data-color="'.$color.'" data-opacity="'.$opacity.'" data-margin="'.$margin.'" data-trigger="'.$icontrigger.'" data-iconvisible="'.$iconvisible.'"  data-tooltipposition="'.$tooltipposition.'">';
        ...
        add_shortcode('cq_vc_profilecard', 'cq_vc_profilecard_func');
