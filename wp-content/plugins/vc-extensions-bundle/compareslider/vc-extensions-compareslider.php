...
if (!class_exists('VC_Extensions_CompareSlider')) {
    class VC_Extensions_CompareSlider{
        function VC_Extensions_CompareSlider() {
            wpb_map(array(
        ...
        function cq_vc_compareslider_func($atts, $content=null) {
          ...
          foreach ($imagesArr as $key => $theimage) {
              ...
              if($imagewidth!=""){
                  $output .= '<img src="'.aq_resize($imageLocation[0], $imagewidth, null, true, true, true).'" class="cq-compareslider-image" alt="">';
              }else{
        ...
        add_shortcode('cq_vc_compareslider', 'cq_vc_compareslider_func');
