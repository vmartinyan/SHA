<?php session_start(); $wl_rcsm_options = get_option('weblizar_rcsm_options'); ?>
<!DOCTYPE html>
<html lang="en">
<head>
	<title></title>	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="keywords" content="<?php echo $wl_rcsm_options['page_meta_keywords']; ?>">
	<meta name="description" content="<?php echo $wl_rcsm_options['page_meta_discription']; ?>">
	<?php if($wl_rcsm_options['search_robots'] == 'on') { ?>
	<meta name="robots" content="<?php echo $wl_rcsm_options['rcsm_robots_meta'];?>"> 
	<?php } ?>
	<link rel="shortcut icon" href="<?php echo esc_url($wl_rcsm_options['upload_image_favicon']);?>">
	<meta name="author" content="">
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	<script src="<?php echo plugin_dir_url( __FILE__ ).'js/scrollReveal.min.js'; ?>"></script>
	<script src="<?php echo plugin_dir_url( __FILE__ ).'js/script-frontend.js' ?>"></script>
	<link rel="stylesheet" href="<?php echo plugin_dir_url( __FILE__ ).'../css/font-awesome.min.css'; ?>">
	<link rel="stylesheet" href="<?php echo plugin_dir_url( __FILE__ ).'css/animate.min.css'; ?>">
	...
	<link rel="stylesheet" href="<?php echo plugin_dir_url( __FILE__ ).'css/media-screen.css'; ?>">
			...
			<?php function page_data(){
						...
						<ul class="animated fadeInDownBig">
							...
							for($i=1; $i<=5; $i++){
								if($wl_rcsm_options['social_icon_'.$i]!=''){ ?>
									<li><a target="<?php if($wl_rcsm_options['link_tab_'.$i]=='on') echo '_blank'; ?>" href="<?php echo $wl_rcsm_options['social_link_'.$i]; ?>"><i class="<?php echo $wl_rcsm_options['social_icon_'.$i].' fa icon'; ?>"></i></a></li><?php
								}
							} ?>	
						</ul>
					</div>
				...
				<div class="container">
					<div class="carousel-caption">
						<h4 class="logo_class"><?php $site_logo_value = $wl_rcsm_options['site_logo'];
							if ($site_logo_value == 'logo_text') { 
								echo $wl_rcsm_options['logo_text_value']; 
							} elseif( $wl_rcsm_options['upload_image_logo'] == null) {
								echo $wl_rcsm_options['logo_text_value'];
							} else { ?>
							<img width="<?php echo $wl_rcsm_options['logo_width'];?>" height="<?php echo $wl_rcsm_options['logo_height'];?>" src="<?php echo esc_url($wl_rcsm_options['upload_image_logo']); ?>">
							<?php } ?>
						</h4>
						<?php $layout_status = $wl_rcsm_options['layout_status'];
						if ($layout_status == 'coming_soon_switch') { ?>
							<h1 data-sr="enter top over 1s and move 110px wait 0.3s"><?php echo $wl_rcsm_options['coming-soon_title'];?></h1>
							<h4 data-sr="enter top over 1s and move 110px wait 0.3s"><?php echo $wl_rcsm_options['coming-soon_sub_title'];?></h4>
							<div class="box1" data-sr="enter left over 1s and move 110px wait 0.3s">
								<div class="box2"></div>
							</div>
							<h3 data-sr="enter bottom over 1s and move 110px wait 0.3s"><?php echo $wl_rcsm_options['coming-soon_message'];?></h3>
							<?php 
						}	
							$button_onoff = $wl_rcsm_options['button_onoff'];
						if($button_onoff == 'on') {
							if($wl_rcsm_options['button_text']) { ?>
								<a class="button_link btn" href="<?php echo $wl_rcsm_options['button_text_link'];?>" data-sr="enter bottom over 1s and move 110px wait 0.3s"><?php echo $wl_rcsm_options['button_text'];?></a><?php
							}
						} ?>
					</div>
				</div>
			...
			<div class="container-fluid copyright">
				<div class="social-icons">
					<ul class="social animated fadeInDownBig">
						<?php for($i=1; $i<=5; $i++){
								if($wl_rcsm_options['social_icon_'.$i]!=''){ ?>
								<li><a target="<?php if($wl_rcsm_options['link_tab_'.$i]=='on') echo '_blank'; ?>" href="<?php echo $wl_rcsm_options['social_link_'.$i]; ?>"><i class="<?php echo $wl_rcsm_options['social_icon_'.$i]; ?> fa icon"></i></a></li>
								<?php 	}
							} ?>
					</ul>
				</div>
				<div class="container">
					<div class="row" data-sr="enter top over 1s and move 110px wait 0.3s">
						<?php echo $wl_rcsm_options['footer_copyright_text'];?> <a href="<?php echo $wl_rcsm_options['footer_link']; ?>"> <?php echo $wl_rcsm_options['footer_link_text'];?></a>
					</div>
				</div>
			</div>
				...
				if (is_user_logged_in() ) {
					...
					if($UserData->roles[0] == 'administrator') {
						...
						if($wl_rcsm_options['admin_link_text']) {
							?><a class="btn left_side_link" href="<?php echo get_admin_url(); ?>" data-sr="enter bottom over 1s and move 110px wait 0.3s" target="_blank"> <?php echo __($admin_link_text,'weblizar');?></a><?php
						}
					}
				}
			}?>
